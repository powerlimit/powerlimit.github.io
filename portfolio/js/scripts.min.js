...
$(function() {
		function modal(windowID, openButton){
			function modalOpen(){
		...
		/*modal('#portfolio-1', '#open-portfolio-1');
		modal('#portfolio-2', '#open-portfolio-2');
		modal('#portfolio-3', '#open-portfolio-3');*/
		var modalDivs = document.querySelectorAll('div[id^=port]'),
			modalOpens= document.querySelectorAll('span[id^=open-]');
		
		for (var i = 0; i < modalDivs.length; i++) {
			modal('#'+$(modalDivs[i]).attr('id'), '#'+$(modalOpens[i]).attr('id'));
		}
		/*function modalWindow(box_modal, href_modal, openFon, closeFon, openWindow, closeWindow){
			function openModal(){
				$(box_modal).removeClass('box_hide');
				$(box_modal+" .fon_black").removeClass(closeFon).addClass(openFon);
				$(box_modal+" .window").removeClass(closeWindow).addClass(openWindow);
			};
			function closeModal(){
				$(box_modal+" .font_black").removeClass(openFon).addClass(closeFon);
				$(box_modal+" .window").removeClass(openWindow).addClass(closeWindow);
				setTimeout(function(){
					$(box_modal).addClass('box_hide')
				},800)
			};
			$(href_modal).click(openModal);
			$(box_modal+' .fon_black, .close_window').click(closeModal);
		};
		modalWindow('#modal-window', '#href_modal_window', 'fadeIn', 'fadeOut', 'bounceInLeft', 'bounceOutRight');*/
	  $('#my-header').particleground({				// "Созвездия на главном экране"
	    dotColor: 'rgba(255, 255, 255, 0.2)',
	    lineColor: 'rgba(255, 255, 255, 0.2)'
	  });
	
	
	$(".typed__message").typed({					// печать текста на главном экране
			strings: ["", "Нужен сайт?", "У меня есть идеи!"],
			typeSpeed: 100,
			backSpeed: 80,
			backDelay: 2000,
			callback: function() {
				setTimeout(function(){
					$('.main--screen__button a').fadeIn('5000')
				}, 1000);
			}
	...
	$(window).scroll(function(){						
		$('.parallax-1').css({'top': -300+$(window).scrollTop()/3});
	});
	$(window).scroll(function(){
		$('.parallax-2').css({'top': -600+$(window).scrollTop()/3});
	})
	$(window).scroll(function(){
		$('.parallax-3').css({'top': 280-$(window).scrollTop()/6});
	})
...
/////////////////////////////// MENU //////////////////////////////
	...
	$(window).scroll(function(){						// изменение высоты меню при скролле
		if($(window).scrollTop()>0){
			$('.top-line').css({'padding': '0'});
		}else{
			$('.top-line').css({'padding': '10px'});
		};
		if($(window).width()<992){
			myNav.css({
				'top': $('.top-line').outerHeight()+'px',
				'height':'calc(100vh - '+$('.top-line').outerHeight()+'px)'
			});
		}
...
//функция выделения активного пункта меню при скролле
	...
	function onScroll(){					
	    ...
	    $(menu_selector).each(function(){
	        ...
	        var target = $(hash);
	        if (target.position().top-50 <= scroll_top && target.position().top + target.outerHeight() > scroll_top) {
